<!--http://localhost:8090/#/ums/addMenu-->
<template>
    ...
    <el-form :model="menu"
      ...
      <el-form-item label="Upper menu：">
        <el-select v-model="menu.parentId"
                   placeholder="Please select the menu">
          <el-option
    ...
    data() {
      return {
        ...
        rules: {
          ...
          name: [
            {required: true, message: 'Please enter the front-end name', trigger: 'blur'},
            {min: 2, max: 140, message: '2 to 140 characters in length', trigger: 'blur'}
          ],
          icon: [
            {required: true, message: 'Please enter the front-end name', trigger: 'blur'},
    ...
    methods: {
      getSelectMenuList() {
        fetchList(0, {pageSize: 100, pageNum: 1}).then(response => {
          ...
          this.selectMenuList.unshift({id: 0, title: 'No superior menu'});
        });
      },
      onSubmit(formName) {
