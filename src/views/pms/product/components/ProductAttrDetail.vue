    ...
    <el-form :model="value" ref="productAttrForm" label-width="120px" style="width: 720px" size="small">
      ...
      <el-form-item label="Specifications：">
        <el-card shadow="never" class="cardBg">
          <div v-for="(productAttr,idx) in selectProductAttr">
            ...
            <div v-else>
              ...
              <el-button class="littleMarginLeft" @click="handleAddProductAttrValue(idx)">Increase</el-button>
            </div>
          </div>
        </el-card>
        ...
        <el-button
          type="primary"
          style="margin-top: 20px"
          @click="handleSyncProductSkuStock">Synchronize inventory
        </el-button>
      </el-form-item>
      <el-form-item label="Property picture：" v-if="hasAttrPic">
        <el-card shadow="never" class="cardBg">
      ...
      <el-form-item style="text-align: center">
        <el-button size="medium" @click="handlePrev">Previous, promotion</el-button>
        <el-button type="primary" size="medium" @click="handleNext">Next, product association</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        //Whether the initialization is successful in edit mode
        hasEditCreated:false,
        //Commodity attribute classification drop-down options
        productAttributeCategoryOptions: [],
        //Selected product attributes
        selectProductAttr: [],
        //Selected product parameters
        selectProductParam: [],
        //Selected product attribute image
    ...
    computed: {
      ...
      //The main picture and album picture of the product
      selectProductPics:{
        get:function () {
          let pics=[];
    ...
    methods: {
      handleEditCreated() {
        //Get attributes and parameters according to product attribute classification id
      ...
      getProductAttrList(type, cid) {
        ...
        fetchProductAttrList(cid, param).then(response => {
          ...
          if (type === 0) {
            ...
            for (let i = 0; i < list.length; i++) {
              ...
              if (this.isEdit) {
                if (list[i].handAddStatus === 1) {
                  //Get manually added editing attributes in editing state
                  options = this.getEditAttrOptions(list[i].id);
                }
                //Get selected attributes in editing state
            ...
            if(this.isEdit){
              //Refresh the product attribute picture in edit mode
      ...
      //Get the set property value that can be manually added
      ...
      handleAddProductAttrValue(idx) {
        ...
        if (options.indexOf(this.addProductAttrValue) !== -1) {
          this.$message({
            message: 'Property value cannot be repeated',
            type: 'warning',
            duration: 1000
          });
          return;
      ...
      handleRefreshProductSkuList() {
        this.$confirm('Refreshing the list will cause the sku information to be regenerated, whether to refresh', 'Hint', {
      ...
      handleSyncProductSkuPrice(){
        this.$confirm('The price of the first sku will be synchronized to all sku, whether to continue', 'Hint', {
          confirmButtonText: 'Sure',
          cancelButtonText: 'Cancel',
          type: 'warning'
        }).then(() => {
      ...
      handleSyncProductSkuStock(){
        this.$confirm('Will synchronize the inventory of the first sku to all sku, whether to continue', 'Hint', {
          confirmButtonText: 'Sure',
          cancelButtonText: 'Cancel',
          type: 'warning'
        }).then(() => {
      ...
      //Combine product attribute images
